...
const positionToStyle = (key: FabPosition, fabSize: number) => {
  ...
  const spacingBetweenButtons = 16; // Space between search and guidance buttons

  const isLeft = key.startsWith('left');
  
  // Calculate guidance button position (right-bottom)
  const guidanceButtonTop = window.innerHeight - fabSize - navHeight - safeBottom - margin;
  
  // For right-bottom, position just above the guidance button
  const vert = key.endsWith('top')
    ...
    : key.endsWith('middle')
      ...
      : key === 'right-bottom'
        ? { top: `${guidanceButtonTop - fabSize - spacingBetweenButtons}px` }
...
const readFabPosition = async (): Promise<FabPosition | null> => {
  ...
  if (data?.user) {
    ...
    const key = (meta?.search_fab_position as string) || '';
    if (key && [
  ...
  const local = localStorage.getItem('search-fab-position-key');
...
const writeFabPosition = async (key: FabPosition) => {
  ...
  if (data?.user) {
    ...
    localStorage.setItem('search-fab-position-key', key);
    return;
  }
  localStorage.setItem('search-fab-position-key', key);
};
